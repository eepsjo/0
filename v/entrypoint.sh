...
if [ -n "$uuid" ]; then
    EFFECTIVE_UUID="$uuid"
    echo "使用用户提供的 UUID: $EFFECTIVE_UUID"
else
    EFFECTIVE_UUID=$(/app/sing-box generate uuid)
    echo "自动生成 UUID: $EFFECTIVE_UUID"
fi
echo "--------------------------------------------------"
...
EOF
echo "sing-box 配置已部署 (端口: ${LISTEN_PORT})"

nohup /app/sing-box run -c 0.json > /dev/null 2>&1 &
SINGBOX_PID=$! # 获取 sing-box 进程ID
sleep 2
...
if [ "$TUNNEL_CONNECTED" = "true" ]; then
    ...
    cat "$LINKS_FILE"
    echo "--------------------------------------------------"
    echo "正在显示隧道日志 (log.log)："
    # 在后台运行 tail -f，以便主脚本可以继续执行更新循环
    tail -f ./log.log &
    TAIL_PID=$! # 获取 tail 进程ID
else
    echo "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
    echo "Cloudflare $TUNNEL_MODE 连接失败"
    echo "请检查日志，并确认配置正确"
    if [ "$TUNNEL_MODE" = "固定隧道" ]; then
        echo "确保 Token 和域名正确，并在 Cloudflare Dashboard 正确配置"
    fi
    echo "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
...
update_binaries() {
    ...
    if [ -z "$CURRENT_SB_VERSION" ]; then
        ...
        CURRENT_SB_VERSION="0.0.0" # 设为旧版本以强制检查更新
    fi
    ...
    if [ -z "$CURRENT_CF_VERSION" ]; then
        ...
        CURRENT_CF_VERSION="0.0.0" # 设为旧版本以强制检查更新
    fi
